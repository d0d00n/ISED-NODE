...
function apijsonRoute() {
    ...
    apijson.get('/api.json', function(req, res) {
        ...
        getMasterJSON();
        lang = req.query.lang.toLowerCase() || 'en';
        ...
        return new Promise(function(resolve, reject) {
            ...
            if (tenant.plansforUser) {
                ...
                if (Array.isArray(tenant.plansforUser)) {
                    // we only care about account plan. there is only one account plan. find it and use it.
                    tenant.plansforUser.forEach(function (item, index){
                        if (item.type === 'account_plan'){
                            planToSend = item;
                        }
                    });
                } else {
                    planToSend = tenant.plansforUser;
                }
                var planCall = geUserAccessByPlans(planToSend);
                planCall.then(function() {
    ...
    function processActiveDocs(tenant, api){
                                ...
                                for (var doc of tenant.docs.api_docs) {
                                    if (doc.api_doc.system_name.toLowerCase() === api.service.system_name.toLowerCase() + '-en'){
                                        hasEn = true;
                                    }
                                    else if (doc.api_doc.system_name.toLowerCase() === api.service.system_name.toLowerCase() + '-fr'){
                                        hasFr = true;
                                    }
                                    if (hasEn && hasFr){
                                        break;
                                ...
                                if (hasEn && hasFr){
                                    for (var doc of tenant.docs.api_docs) {
                                        if (doc.api_doc.system_name.toLowerCase() === api.service.system_name.toLowerCase() + '-' + lang && doc.api_doc.published === true) {
                                            var apiToAdd = {};
                                            if (cache.get(tenant.name + '-' + api.service.id + '-' + lang+'.name') == null){
                                                ...
                                                if (checkApiValue(swaggerbody.info.contact)) {
                                                    ...
                                                    cache.put(tenant.name + '-' + api.service.id + '-' + lang+'.contactName', swaggerbody.info.contact.name, cacheMin * 60000);
                                                    apiToAdd.contact.email = swaggerbody.info.contact.email;
                                                    cache.put(tenant.name + '-' + api.service.id + '-' + lang+'.contactEmail', swaggerbody.info.contact.email, cacheMin * 60000);
                                                }
